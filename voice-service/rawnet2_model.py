"""RawNet2 anti-spoofing model architecture (Tak et al., ICASSP 2021,
"End-to-end anti-spoofing with RawNet2"), vendored (not pip-installed) so we depend on
only this one model rather than pulling in the full Jabberjay package's much heavier
transitive dependencies (transformers, matplotlib, datasets, ...) for the other nine model
families we don't use -- see app/voice/detector.py for why RawNet2 specifically was chosen
(67MB checkpoint vs. 361MB-1.2GB for the other options that scored comparably in testing).

Original implementation: https://github.com/eurecom-asp/rawnet2-antispoofing (MIT license).
This copy is taken from MattyB95/Jabberjay (also MIT), which bundles it unchanged.
"""
...
class SincConv(nn.Module):
    ...
    def __init__(
        ...
        hamming = torch.tensor(
            np.hamming(self.kernel_size), device=self.device, dtype=torch.float32
        )
        band_pass = torch.zeros(self.out_channels, self.kernel_size, device=self.device)
